		     ...
		     *per_conversation_info)
{
	...
	case VNC_SESSION_STATE_TIGHT_TUNNELING_CAPABILITIES :
	{
		...
		for(i = 0; i < num_tunnel_types; i++) {
			...
			proto_tree_add_item(tree, hf_vnc_tight_tunnel_type_vendor, tvb, offset + 4, 4, ENC_BIG_ENDIAN);
			proto_tree_add_item(tree, hf_vnc_tight_tunnel_type_signature, tvb, offset + 8, 8, ENC_BIG_ENDIAN);
			offset += 16;
		}
